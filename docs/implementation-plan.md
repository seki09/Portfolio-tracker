...
Local Docker is the primary target, but the app is built to also run on a serverless host
(Netlify + managed Postgres) without a rewrite. The five seams that make that true are
specified in [deployment-portability.md](deployment-portability.md) and are folded into the
milestones below; this document flags them inline as **[portability]**.
...
└───────┬───────┘                  │                           │
        └──────────────┬───────────┘                    external HTTP
             ...
             └─────────┬──────────┘
                       ▼
             ┌────────────────────┐        ┌──────────────────────────┐
             │ PostgreSQL 16      │        │ scheduler                │
             │  ledger + caches   │◄───────│  node-cron (local) or    │
             └────────────────────┘        │  Netlify Scheduled Fns   │
                       ▲                   │  → POST /api/jobs/:name  │
                       │                   └──────────────────────────┘
             ┌─────────┴──────────┐
             │ BlobStore          │  disk volume (local) │ Netlify Blobs / S3
             │  uploaded files    │
             └────────────────────┘
```

Three containers in `docker compose`: `web`, `scheduler`, `db`. Uploaded files live behind
a `BlobStore` interface — a mounted volume locally — not in the database, because they are
re-parseable evidence and keeping them out of Postgres keeps backups small and `pg_dump`
fast.

**[portability]** The scheduler does not *contain* the jobs; it only triggers them over
HTTP. The jobs themselves are idempotent, resumable route handlers that live in the web
app. That one decision is what lets a Netlify Scheduled Function replace the container with
no code change.
...
- **A separate scheduler** so a slow price provider can never block a page render, and so
  the nightly fetch runs whether or not a browser is open.
- **[portability] Every platform dependency behind an interface** — database driver, blob
  storage, job scheduling, config, sessions. Five small seams, roughly four days of extra
  work spread across M0–M3, versus rewriting the import pipeline and every job later. Each
  also pays for itself locally: async parsing gives progress indicators, HTTP-triggered jobs
  give a "Run now" button, validated config catches misconfiguration at boot.

### Two targets, one codebase

| | Local (primary) | Serverless (future) |
| --- | --- | --- |
| Web | `web` container | Netlify + `@netlify/plugin-nextjs` |
| Database | `db` container (Postgres 16) | Neon / Supabase, pooled connection |
| Jobs | `scheduler` container, node-cron | Netlify Scheduled Functions |
| Files | mounted volume | Netlify Blobs or S3-compatible |
| Selected by | `DEPLOY_TARGET=container` | `DEPLOY_TARGET=serverless` |

Fly.io, Railway and Vercel are reachable from the same seams; none of them needs its own
code path. See [deployment-portability.md](deployment-portability.md) for the interfaces,
the rules the code must follow, and the privacy trade-off cloud hosting involves.
...
| Jobs | HTTP handlers + `node-cron` trigger | **[portability]** Chunked, resumable, idempotent; a queue (BullMQ) would be both overkill and un-portable to serverless |
| Storage | `BlobStore` interface (disk / Netlify Blobs / S3) | **[portability]** Ephemeral filesystems on serverless make direct disk writes a dead end |
| Config | zod-validated runtime module | Fails fast on a missing secret instead of half-working |
...
**[portability]** Jobs are not methods on a long-lived worker. Each is an idempotent,
chunked, resumable handler exposed at `POST /api/jobs/:name` behind a shared-secret header:
...
A job processes a bounded slice — 25 securities, 90 days of series — and returns a cursor;
the caller re-invokes until `done`. That keeps every invocation inside a ~10 s serverless
budget without depending on background functions, and locally it means a long backfill
reports progress instead of blocking. The scheduler is then interchangeable: node-cron in a
container, a Netlify Scheduled Function, an external cron, or the **"Run now" button** in
Settings → System — which is worth building purely for local debuggability.
...
| Import parse | on upload | **[portability]** Parse a staged file into `staged_transaction` rows; the review screen polls while `import_batch.status = PARSING` |
| Backup | daily (optional) | `pg_dump` + blobs → a configured destination, with rotation |
...
Parsing as a job rather than inside the upload request is the change that makes large files
safe on both targets — a 5,000-row XLSX or a scanned-length PDF cannot time out a request
...
- **Auth:** a password set via environment variable, argon2-hashed, with a signed,
  HTTP-only, `Secure` session cookie carrying only an expiry and a version counter — **no
  session table**, so it works identically on both targets. Locally, the deployment docs
  should be blunt that this belongs behind a VPN or a reverse proxy with TLS, not on the
  open internet. On a public cloud deploy there is no VPN in front of it, so cloud mode
  should enforce a real password length and consider TOTP before it is used in anger.
...
- **[portability] Cloud hosting trades part of that away, and the docs must say so.** Your
  full transaction history would sit on someone else's infrastructure under their keys.
  Broker PDFs are the sharp edge — they carry account numbers, IBANs, your address and tax
  ID — so serverless mode defaults to **parse-then-discard**: keep the transactions and the
  file hash for duplicate detection, drop the source file. Retention becomes an explicit
  opt-in, encrypted client-side when enabled. See
  [deployment-portability.md §5](deployment-portability.md).
- **Job endpoints are authenticated by a separate `JOB_SECRET`**, not the user session, and
  rate-limited. They are the one unauthenticated-by-cookie surface in the app.
...
M7 is genuinely optional and deliberately unscheduled — because the seams land in M0–M3,
it can be done in an afternoon-sized burst whenever you want cloud access, rather than
...
| Platform-coupled code creeps in, quietly killing portability | M7 becomes a rewrite | The §3 rules in the portability doc, ESLint-enforced where possible, plus CI grep for `fs` writes and edge-runtime imports |
| Serverless cold starts / function timeouts | Slow first load, failed jobs | Cursor-chunked jobs never approach the limit; cold start is ~1–2 s and acceptable for this usage — and the same build runs as a container on Fly if not |
| Managed Postgres connection exhaustion | Errors under trivial load | HTTP driver + pooled connection string in serverless mode; no cross-request transactions |
| Sensitive broker PDFs stored on third-party infrastructure | Real privacy exposure | Parse-then-discard by default in cloud mode; opt-in retention encrypted client-side |
